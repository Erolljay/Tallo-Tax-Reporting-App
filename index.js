...
const payrollHeaders = ["Pay_Day", "Employee Name", "Category", "Basic Pay", "Holiday Pay", "Overtime Pay", "Nigh Shift Differential", "De Minimis", "13th Month", "Gross Pay", "SSS - ER/EC", "SSS - EE", "PHIC - ER", "PHIC - EE", "HDMF - ER", "HDMF - EE", "Taxable Pay", "Tax Withheld", "Net Pay", "SSS,PHIC,HDMF - EE", "Account Title", "Contribution Expense"];
...
function displayContentBasedOnSelection(targetId, wrapperEl, title) {
    ...
    try {
            ...
            else if (targetId === 'report-general-ledger') mainContentHtml = generateGeneralLedgerReport(rType, period, year); // New
...
function generatePayrollSummaryReport(reportingType, period, year) {
        ...
        const periodRows = [];
        let grandGross = 0, grandNTDisplay = 0, grandTDisplay = 0, grandWH = 0, grandNP = 0, grandContribExp = 0;
        ...
        for (const name in empSummaries) {
            ...
            if (empData.p_gp === 0 && empData.p_wh === 0 && empData.p_np === 0 && empData.p_contrib_exp === 0 && empData.p_dm === 0 && empData.p_13m === 0 && empData.p_bp === 0) continue;
            ...
            periodRows.push({
                ...
                p_wh: empData.p_wh, p_np: empData.p_np, p_contrib_exp: empData.p_contrib_exp,
        ...
        let tblHtml = `<div class="results-table-wrapper"><table class="results-table"><thead>
            <tr>
                ...
                <th rowspan="2">Tax W/h (Period)</th><th rowspan="2">Net Pay (Period)</th><th rowspan="2">Contrib Exp (Period)</th>
        ...
        periodRows.forEach(r => {
            tblHtml += `<tr>
                ...
                <td class="number">${nf(r.p_wh)}</td><td class="number">${nf(r.p_np)}</td><td class="number">${nf(r.p_contrib_exp)}</td>
        ...
        tblHtml += `</tbody><tfoot><tr>
            ...
            <td class="number font-semibold">${nf(grandWH)}</td><td class="number font-semibold">${nf(grandNP)}</td><td class="number font-semibold">${nf(grandContribExp)}</td>
...
function generateReconWHTReport(reportingType, period, year) {
    console.log(`Entering generateReconWHTReport (Header "As per SLP")... Type: ${reportingType}, Period: ${period}, Year: ${year}`);
    if (!processedData.purchasesExpenses || processedData.purchasesExpenses.length === 0) {
        return '<p class="text-gray-600">No Purchases/Expenses data available.</p>';
    }
    ...
    // This object will store summaries for ALL accounts to correctly calculate "As per SLP"
    const allAccountsSummary = { opex: {}, capex: {} };
    const itemsWithoutWHT_for_SLP_Supporting = []; // For the supporting SLP report

    periodFilteredData.forEach(row => {
        const accountTitle = row.ACCOUNT_TITLE_PURCH?.trim() || "Uncategorized Expenses";
        const category = getExpenseCategoryForReconWHT(accountTitle);
        const vatable = safeParseFloat(row.VATABLE);
        const nonVatable = safeParseFloat(row.NON_VATABLE);

        // Calculate (Vatable/1.12 + NonVatable) for every row, as this forms the basis for "As per SLP"
        ...
        if (!targetSummaryGroup[accountTitle]) {
            targetSummaryGroup[accountTitle] = {
                sum_totalPotentialBase_AsPerSLP: 0, // Will be used for "As per SLP" column
                sum_baseSubjectedToWHT: 0,         // For "Tax Base (Subjected to WHT)" column
                sum_amountWithheld: 0              // For "Amount Withheld" column
            };
        }

        // Accumulate this for the "As per SLP" column
        ...
        if (withholdingTax > 0) {
            // If this row had WHT, add its base to sum_baseSubjectedToWHT for the "Tax Base" column
            ...
            // This item did not have WHT, so collect it for the supporting SLP report
            itemsWithoutWHT_for_SLP_Supporting.push(row);
        }
    });

    // Prepare items for the main report table (only accounts that had some WHT)
    const opexItemsForMainReport = {};
    for (const accTitle in allAccountsSummary.opex) {
        const data = allAccountsSummary.opex[accTitle];
        if (data.sum_amountWithheld > 0) { // Only include if WHT actually occurred for this account
            opexItemsForMainReport[accTitle] = {
                taxBase: data.sum_baseSubjectedToWHT,
                amountWithheld: data.sum_amountWithheld,
                asPerSLP_TotalBase: data.sum_totalPotentialBase_AsPerSLP, // This is our "As per SLP" value
    ...
    mainReportHtml += '<p class="text-sm text-gray-600 mb-3">This report includes purchases/expenses that were subjected to withholding tax, summarized by Account Title.</p>';
    ...
    const generateSummarizedSectionTable = (title, accountGroupItems) => {
        ...
        if (accountTitles.length === 0) {
            return { html: sectionHtml + '<p class="text-gray-500">No items for this category with withholding tax.</p>', totals: { totalTaxBase: 0, totalAmountWithheld: 0, totalAsPerSLP: 0, totalDifference: 0 } };
        }
        ...
        sectionHtml += `<div class="results-table-wrapper"><table class="results-table">
            <thead>
                <tr>
                    ...
                    <th class="number">As per SLP (Total Potential Base)</th> <!-- UPDATED HEADER -->
        ...
        accountTitles.sort().forEach(accTitle => {
            ...
            totalAsPerSLP += item.asPerSLP_TotalBase;
    ...
    const opexResult = generateSummarizedSectionTable('Operational Expenditures (OPEX) - Subject to WHT', opexItemsForMainReport);
    const capexResult = generateSummarizedSectionTable('Capital Expenditures (CAPEX) - Subject to WHT', capexItemsForMainReport);
    ...
    mainReportHtml += `<h4 class="text-md font-semibold mt-6 mb-2">Grand Totals - Subject to WHT</h4>
        <div class="results-table-wrapper">
        ...
        <p class="text-xs text-gray-500 mt-2">
            ...
            * As per SLP (Total Potential Base) = Sum of [(Vatable Purchases / 1.12) + Non-Vatable Purchases] for ALL items under the account title within the period, regardless of WHT status.<br/> {/* UPDATED NOTE */}
            * Difference = As per SLP (Total Potential Base) - Tax Base (Subjected to WHT).
        </p>`;
    ...
    let supportingReportHtml = `<div class="supporting-details mt-8 pt-4 border-t border-dashed border-gray-300">
        ...
        <p class="text-sm text-gray-600 mb-3">This report is in Summary List of Purchases (SLP) format.</p>`;
    ...
    if (itemsWithoutWHT_for_SLP_Supporting.length === 0) {
        ...
        let grandTotalGrossVatable_supp = 0;
        let grandTotalVatableNet_supp = 0;
        let grandTotalExempt_supp = 0;
        let grandTotalVat_supp = 0;

        const periodGroupedData_supp = {};
        itemsWithoutWHT_for_SLP_Supporting.forEach(row => { // Use the correctly filtered list
            ...
            const inputVat = safeParseFloat(row.VAT_PURCH);

            if (!isNaN(v)) periodGroupedData_supp[key].periodVatableSum += v;
            if (!isNaN(nv)) periodGroupedData_supp[key].periodNonVatableSum += nv;
            if (!isNaN(inputVat) && inputVat > 0) {
                    periodGroupedData_supp[key].periodInputVatSum += inputVat;
            }
        });
        ...
        for (const key in periodGroupedData_supp) {
            ...
            const finalVat = (group.periodInputVatSum > 0) ? group.periodInputVatSum : (vatableNet * 0.12);
            ...
            supportingReportHtml += `</tbody>
                <tfoot>
                    <tr class="font-semibold" style="background-color: #e5e7eb;">
                        ...
                        <td class="number">${nf(grandTotalGrossVatable_supp)}</td>
                        <td class="number">${nf(grandTotalVatableNet_supp)}</td>
                        <td class="number">${nf(grandTotalExempt_supp)}</td>
                        <td class="number">${nf(grandTotalVat_supp)}</td>
                    </tr>
                </tfoot>
            </table></div>
            <p class="text-xs text-gray-500 mt-2">* Vatable Purchase (Net) = Gross Vatable Purchase / 1.12. Exempt Purchase = Non-Vatable from source. Input VAT uses 'VAT' column from source if present and positive, else calculated from Vatable Purchase (Net).</p>`;
        }
    }
...
// --- Helper Function (can be placed globally or near other report generators) ---
// You can reuse the CAPEX_KEYWORDS from your generateGeneralLedgerReport or define it here
const RECON_WHT_CAPEX_KEYWORDS = [
    ...
    "WORK IN PROGRESS - CAPEX", "CONSTRUCTION IN PROGRESS", // Keywords from your generalLedger function
    "IMPROVEMENT", "DEPOSIT" // Additional specific keywords if needed for WHT recon
];

function getExpenseCategoryForReconWHT(accountTitle) {
    if (!accountTitle) return 'OPEX'; // Default to OPEX if no title
    const titleUpper = accountTitle.toUpperCase();
    for (const keyword of RECON_WHT_CAPEX_KEYWORDS) {
        if (titleUpper.includes(keyword.toUpperCase())) { // Ensure keyword is also uppercase for comparison
            return 'CAPEX';
        }
    }
    return 'OPEX';
